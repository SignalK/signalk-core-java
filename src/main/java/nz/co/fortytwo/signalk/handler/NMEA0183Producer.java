...
public class NMEA0183Producer {
    ...
    public String createMWVApparent(SignalKModel model) {
        if (model.getValue(vessels_dot_self_dot + env_wind_angleApparent) != null
            ...
            sen.setStatus(DataStatus.ACTIVE);
            if (model.getValue(vessels_dot_self_dot + env_wind_angleApparent) != null) {
                // convert SignalK angles [-PI...PI] to NMEA [0...360]
                double appWind = (double) model.getValue(vessels_dot_self_dot + env_wind_angleApparent);
                if (appWind < 0) {
                    appWind += + 2.0*Math.PI;
                }
                if (logger.isDebugEnabled()) {
                    logger.debug("Apparent dir:" + appWind);
                }
                sen.setAngle(Math.toDegrees(appWind));
            }
            sen.setTrue(false);
    ...
    public String createMWVTrue(SignalKModel model) {
        if (model.getValue(vessels_dot_self_dot + env_wind_directionTrue) != null
            ...
            if (model.getValue(vessels_dot_self_dot + env_wind_directionTrue) != null) {
                // convert SignalK angles [-PI...PI] to NMEA [0...360]
                double truWind = (double) model.getValue(vessels_dot_self_dot + env_wind_directionTrue);
                if (truWind < 0) {
                    truWind += 2.0*Math.PI;
                }
                sen.setAngle(Math.toDegrees(truWind));
            }
            sen.setTrue(true);
