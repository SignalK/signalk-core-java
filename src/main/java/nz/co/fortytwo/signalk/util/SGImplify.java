...
public class SGImplify {
	 ...
	 * @return
	 */
	public static Json simplifyLine2D(double tol, Json track) {
		Tuple2f [] tuples = toTuples(track);
		Tuple2f []  result = simplifyLine2D(tol, tuples);
		return toJson(result);
	}
	
	private static Json toJson(Tuple2f[] result) {
		Json json = Json.array();
		for(Tuple2f tp: result){
			json.add(Json.array(tp.x,tp.y));
		}
		return json;
	}

	private static Tuple2f[] toTuples(Json track) {
		ArrayList<Tuple2f> tuples = new ArrayList<Tuple2f>();
		for(Json wp: track.asJsonList()){
